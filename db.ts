...
if (countTypes.count === 0) {
  db.transaction(() => {
    ...
    const infoTuboCuadMec = insertType.run('Tubo Cuadrado Mecánico');
  ...
  // Tubo Cuadrado Mecánico
  const sqMec = (b: number, e: number, a: number, p: number, i: number, w: number) => 
    insertProfile.run(infoTuboCuadMec.lastInsertRowid, `${b}x${b}x${e}`, b, b, e, 0, a, i, w, i, w, p);

  sqMec(12, 0.75, 0.32, 0.26, 0.07, 0.11);
  sqMec(12, 0.90, 0.38, 0.30, 0.08, 0.13);
  sqMec(12, 1.00, 0.41, 0.33, 0.08, 0.13);
  sqMec(12, 1.10, 0.45, 0.36, 0.09, 0.14);
  
  sqMec(15, 0.75, 0.41, 0.33, 0.16, 0.19);
  sqMec(15, 0.90, 0.49, 0.39, 0.19, 0.21);
  
  sqMec(20, 0.60, 0.46, 0.36, 0.34, 0.29);
  sqMec(20, 0.75, 0.56, 0.45, 0.40, 0.34);
  sqMec(20, 0.90, 0.67, 0.53, 0.48, 0.41);
  sqMec(20, 1.10, 0.80, 0.64, 0.54, 0.47);
  sqMec(20, 1.50, 1.05, 0.84, 0.65, 0.58);
  
  sqMec(25, 0.75, 0.71, 0.56, 0.82, 0.55);
  sqMec(25, 0.90, 0.85, 0.67, 1.00, 0.67);
  sqMec(25, 1.10, 1.02, 0.81, 1.12, 0.76);
  sqMec(25, 1.50, 1.35, 1.08, 1.39, 0.97);
  
  sqMec(30, 0.90, 1.03, 0.81, 1.79, 1.00);
  sqMec(30, 1.10, 1.24, 0.98, 2.01, 1.13);
  sqMec(30, 1.50, 1.65, 1.31, 2.54, 1.47);
  
  sqMec(40, 1.10, 1.68, 1.33, 5.03, 2.10);
  sqMec(40, 1.50, 2.25, 1.79, 6.48, 2.74);
  
  sqMec(50, 1.20, 2.31, 1.82, 9.06, 3.63);
  sqMec(50, 1.50, 2.85, 2.26, 11.06, 4.42);
  ...
  const roundMec = (label: string, d: number, e: number, a: number, p: number, i: number, w: number) =>
    insertProfile.run(infoTuboRedMec.lastInsertRowid, `${label} (${e}mm)`, d, 0, e, 0, a, i, w, i, w, p);

  roundMec('1/2"', 12.70, 0.95, 0.35, 1.86/6, 0.06, 0.09);
  roundMec('1/2"', 12.70, 1.10, 0.40, 2.10/6, 0.07, 0.11);
  roundMec('1/2"', 12.70, 1.50, 0.53, 2.82/6, 0.08, 0.13);
  roundMec('5/8"', 15.88, 0.95, 0.44, 2.28/6, 0.12, 0.15);
  roundMec('5/8"', 15.88, 1.10, 0.51, 2.64/6, 0.14, 0.18);
  roundMec('5/8"', 15.88, 1.50, 0.68, 3.54/6, 0.18, 0.22);
  roundMec('3/4"', 19.05, 0.95, 0.54, 2.70/6, 0.22, 0.23);
  roundMec('3/4"', 19.05, 1.10, 0.62, 3.18/6, 0.25, 0.26);
  roundMec('3/4"', 19.05, 1.50, 0.83, 4.20/6, 0.32, 0.34);
  roundMec('7/8"', 22.22, 0.95, 0.63, 3.18/6, 0.36, 0.32);
  roundMec('7/8"', 22.22, 1.10, 0.73, 3.66/6, 0.41, 0.37);
  roundMec('7/8"', 22.22, 1.50, 0.98, 4.92/6, 0.53, 0.47);
  roundMec('1"', 25.40, 0.95, 0.73, 3.60/6, 0.55, 0.43);
  roundMec('1"', 25.40, 1.10, 0.84, 4.20/6, 0.62, 0.49);
  roundMec('1"', 25.40, 1.50, 1.13, 5.64/6, 0.81, 0.64);
  roundMec('1 1/4"', 31.75, 0.95, 0.92, 4.50/6, 1.09, 0.69);
  roundMec('1 1/4"', 31.75, 1.10, 1.06, 5.22/6, 1.24, 0.78);
  roundMec('1 1/4"', 31.75, 1.50, 1.43, 7.08/6, 1.63, 1.03);
  roundMec('1 1/2"', 38.10, 0.95, 1.11, 5.40/6, 1.91, 1.00);
  roundMec('1 1/2"', 38.10, 1.10, 1.28, 6.24/6, 2.19, 1.15);
  roundMec('1 1/2"', 38.10, 1.50, 1.72, 8.46/6, 2.89, 1.52);
  roundMec('1 3/4"', 44.45, 0.95, 1.30, 6.24/6, 3.07, 1.38);
  roundMec('1 3/4"', 44.45, 1.10, 1.50, 7.26/6, 3.52, 0.16);
  roundMec('1 3/4"', 44.45, 1.50, 2.02, 9.84/6, 4.67, 2.10);
  roundMec('1 7/8"', 47.63, 0.95, 1.40, 6.78/6, 3.80, 1.60);
  roundMec('1 7/8"', 47.63, 1.10, 1.61, 7.80/6, 4.35, 1.83);
  roundMec('1 7/8"', 47.63, 1.50, 2.17, 10.26/6, 5.79, 2.43);
  roundMec('2"', 50.80, 0.95, 1.49, 7.20/6, 4.62, 1.82);
  roundMec('2"', 50.80, 1.10, 1.72, 8.34/6, 5.30, 2.09);
  roundMec('2"', 50.80, 1.50, 2.32, 10.80/6, 7.06, 2.78);
  roundMec('2 3/8"', 60.33, 1.50, 2.77, 13.20/6, 12.00, 3.98);
  roundMec('2 1/2"', 63.50, 1.50, 2.92, 14.04/6, 14.05, 4.42);

  // Tubo Redondo Estructural
  const roundEst = (label: string, d: number, e: number, a: number, p: number, i: number, w: number) =>
    insertProfile.run(infoTuboRedEst.lastInsertRowid, `${label} (${e}mm)`, d, 0, e, 0, a, i, w, i, w, p);

  roundEst('3/4"', 19.10, 1.40, 0.78, 0.61, 0.30, 0.32);
  roundEst('3/4"', 19.10, 1.50, 0.83, 0.65, 0.32, 0.34);
  roundEst('3/4"', 19.10, 1.80, 0.98, 0.77, 0.37, 0.39);
  roundEst('3/4"', 19.10, 2.00, 1.07, 0.84, 0.39, 0.41);
  
  roundEst('7/8"', 22.20, 1.40, 0.92, 0.72, 0.50, 0.45);
  roundEst('7/8"', 22.20, 1.50, 0.98, 0.77, 0.53, 0.47);
  roundEst('7/8"', 22.20, 1.80, 1.16, 0.91, 0.61, 0.55);
  roundEst('7/8"', 22.20, 2.00, 1.27, 1.00, 0.66, 0.59);
  
  roundEst('1"', 25.40, 1.40, 1.06, 0.83, 0.76, 0.60);
  roundEst('1"', 25.40, 1.50, 1.13, 0.88, 0.81, 0.64);
  roundEst('1"', 25.40, 1.80, 1.33, 1.05, 0.93, 0.74);
  roundEst('1"', 25.40, 2.00, 1.47, 1.15, 1.01, 0.80);
  
  roundEst('1 1/4"', 31.80, 1.40, 1.33, 1.05, 1.54, 0.97);
  roundEst('1 1/4"', 31.80, 1.50, 1.43, 1.12, 1.63, 1.03);
  roundEst('1 1/4"', 31.80, 1.80, 1.69, 1.33, 1.91, 1.20);
  roundEst('1 1/4"', 31.80, 2.00, 1.87, 1.47, 2.08, 1.31);
  
  roundEst('1 1/2"', 38.10, 1.40, 1.61, 1.27, 2.72, 1.43);
  roundEst('1 1/2"', 38.10, 1.50, 1.72, 1.35, 2.89, 1.52);
  roundEst('1 1/2"', 38.10, 1.80, 2.05, 1.61, 3.39, 1.78);
  roundEst('1 1/2"', 38.10, 2.00, 2.27, 1.78, 3.71, 1.95);
  
  roundEst('1 3/4"', 44.50, 1.40, 1.89, 1.49, 4.39, 1.98);
  roundEst('1 3/4"', 44.50, 1.50, 2.02, 1.59, 4.67, 2.10);
  roundEst('1 3/4"', 44.50, 1.80, 2.41, 1.89, 5.49, 2.47);
  roundEst('1 3/4"', 44.50, 2.00, 2.67, 2.09, 6.02, 2.71);
  
  roundEst('1 7/8"', 47.60, 1.40, 2.03, 1.60, 5.44, 2.28);
  roundEst('1 7/8"', 47.60, 1.50, 2.17, 1.71, 5.79, 2.43);
  roundEst('1 7/8"', 47.60, 1.80, 2.59, 2.03, 6.81, 2.86);
  roundEst('1 7/8"', 47.60, 2.00, 2.87, 2.25, 7.47, 3.14);
  
  roundEst('2"', 50.80, 1.40, 2.17, 1.71, 6.63, 2.61);
  roundEst('2"', 50.80, 1.50, 2.32, 1.82, 7.06, 2.78);
  roundEst('2"', 50.80, 1.80, 2.77, 2.18, 8.33, 3.28);
  roundEst('2"', 50.80, 2.00, 3.07, 2.41, 9.14, 3.60);
  roundEst('2"', 50.80, 3.00, 4.51, 3.54, 12.92, 5.09);
  roundEst('2"', 50.80, 4.00, 5.88, 4.62, 16.22, 6.39);
  
  roundEst('2 3/8"', 60.30, 1.50, 2.77, 2.18, 12.00, 3.98);
  roundEst('2 3/8"', 60.30, 1.80, 3.31, 2.60, 14.18, 4.70);
  roundEst('2 3/8"', 60.30, 2.00, 3.66, 2.88, 15.60, 5.17);
  roundEst('2 3/8"', 60.30, 3.00, 5.40, 4.24, 22.25, 7.38);
  roundEst('2 3/8"', 60.30, 4.00, 7.08, 5.56, 28.21, 9.35);
  
  roundEst('2 1/2"', 63.50, 1.50, 2.92, 2.29, 14.05, 4.42);
  roundEst('2 1/2"', 63.50, 1.80, 3.49, 2.74, 16.62, 5.23);
  roundEst('2 1/2"', 63.50, 2.00, 3.86, 3.03, 18.29, 5.76);
  roundEst('2 1/2"', 63.50, 3.00, 5.70, 4.48, 26.15, 8.24);
  roundEst('2 1/2"', 63.50, 4.00, 7.48, 5.87, 33.24, 10.47);
  
  roundEst('3"', 76.20, 1.50, 3.52, 2.76, 24.56, 6.45);
  roundEst('3"', 76.20, 1.80, 4.21, 3.30, 29.13, 7.65);
  roundEst('3"', 76.20, 2.00, 4.66, 3.66, 32.11, 8.43);
  roundEst('3"', 76.20, 3.00, 6.90, 5.42, 46.29, 12.15);
  roundEst('3"', 76.20, 4.00, 9.07, 7.12, 59.30, 15.56);
  roundEst('3"', 76.20, 5.00, 11.18, 8.78, 71.22, 18.69);
  
  roundEst('4"', 101.60, 2.00, 6.26, 4.91, 77.63, 15.28);
  roundEst('4"', 101.60, 3.00, 9.29, 7.29, 113.04, 22.25);
  roundEst('4"', 101.60, 4.00, 12.26, 9.63, 146.28, 28.80);
  roundEst('4"', 101.60, 5.00, 15.17, 11.91, 177.47, 34.93);
  roundEst('4"', 101.60, 6.00, 18.02, 14.15, 206.68, 40.68);
  
  roundEst('5"', 127.00, 2.00, 7.85, 6.17, 153.44, 24.16);
  roundEst('5"', 127.00, 3.00, 11.69, 9.17, 224.75, 35.39);
  roundEst('5"', 127.00, 4.00, 15.46, 12.13, 292.61, 46.08);
  roundEst('5"', 127.00, 5.00, 19.16, 15.04, 357.14, 56.24);
  roundEst('5"', 127.00, 6.00, 22.81, 17.90, 418.44, 65.90);
  
  roundEst('6"', 152.40, 3.00, 14.08, 11.05, 393.01, 51.58);
  roundEst('6"', 152.40, 4.00, 18.65, 14.64, 513.73, 67.42);
  roundEst('6"', 152.40, 5.00, 23.15, 18.18, 629.54, 82.62);
  roundEst('6"', 152.40, 6.00, 27.60, 21.66, 740.57, 97.19);
  
  roundEst('6 5/8"', 168.30, 3.00, 15.58, 12.23, 532.28, 63.25);
